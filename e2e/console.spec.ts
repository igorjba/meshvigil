...
test.describe("MeshVigil console", () => {
  test("boots the client-side engine and streams telemetry", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Topologia da rede" })).toBeVisible();
  ...
  test("chaos injection produces events and moves the SLA", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /Derrubar coletor/ }).click();

    // The event log should record the collector going offline.
    await expect(page.getByText(/fora do ar/i).first()).toBeVisible({ timeout: 10_000 });

    // Restore should heal it and log a recovery-class event.
    await page.getByRole("button", { name: /Restaurar tudo/ }).click();
    await expect(page.getByText(/recuperando|removidas/i).first()).toBeVisible({ timeout: 10_000 });
  });

  test("DLMS inspector decodes a frame into OBIS registers", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("heading", { name: /Inspetor DLMS \/ COSEM/ })).toBeVisible();
    ...
    await page.getByRole("button", { name: /Frame corrompido/ }).click();
    await expect(page.getByText(/FCS falhou/i).first()).toBeVisible({ timeout: 10_000 });
  });
});
